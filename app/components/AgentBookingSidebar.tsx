...
export default function AgentBookingSidebar({
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            // 3. Log advance payment explicit log if needed (legacy behavior from original file kept just in case, but api.createBooking with advanceAmount usually handles it. 
            // However, the original agent sidebar had an explicit logPayment call. I will rely on the API processing the advanceAmount unless targetBookingId exists and we are in edit mode?
            // Actually, the original BookingSidebar relied on the backend handling `advanceAmount` in createBooking. 
            // The AgentBookingSidebar had explicit logging. I'll stick to the BookingSidebar logic (sending advanceAmount in request) as it's cleaner, assuming backend supports it.
            // If the user specifically wanted the explicit log behavior preserved, I might need to add it back. But "aligning with owner flow" implies using the owner's logic.
            // I'll stick to the cleaner owner logic which sends advance fields.
    ...
    return (
        <div className="bg-white rounded-[2rem] border border-slate-100 shadow-lg overflow-hidden animate-slide-up">
            {/* Header */}
                    ...
                    <div className="space-y-4 animate-in fade-in duration-300">
                        ...
                        <div>
                            ...
                            <div className="relative">
                                ...
                                <input
                                    type="number"
                                    className="w-full bg-emerald-50 border border-emerald-100 rounded-xl pl-7 pr-4 py-3 text-sm font-bold text-emerald-700 outline-none focus:border-emerald-500 transition-colors"
                                    value={formData.agentCommission || ""}
                                    onChange={(e) => handleNumberChange("agentCommission", e.target.value)}
                                />
                            </div>
                        </div>
                                     <div>
                    ...
                    <div className="space-y-4 animate-in fade-in duration-300">
                        ...
                        <div className="bg-[#0D7A6B]/10 rounded-xl p-4 space-y-2">
                            ...
                            {formData.agentCommission > 0 && (
                                <div className="flex justify-between items-center">
                                    <span className="text-xs text-emerald-600">Your Commission</span>
                                    <span className="text-sm font-bold text-emerald-600">{formatCurrency(formData.agentCommission)}</span>
                                </div>
                            )}
